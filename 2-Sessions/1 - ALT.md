---
tags:
  - Session
date:
Players:
DM: "[[Jannis]]"
---

![[Einleitungstext#Einleitung Wächter der Dalelands]]

### 1. Der Auftrag: Routine im Cormanthor
**Situation:** Ihr wurdet als Begleitschutz angeheuert. Niemand geht ohne Waffe tief in den Wald – das war schon immer so.
**Ort:** Waldrand von [[4-Faerûn/Orte/Cormanthor]], nahe [[Wissen - Ashabenford]].
	Etwa 2 Stunden Fußmarsch (~3,5km) in den Wald hinein
	2 Ochsen ziehen den Klippwagen (Logging Wheels), an dem die gefällten Baume hängen
	![[Pasted image 20251203075333.png|100]]

Ihr lehnt an den Bäumen oder haltet Wache?
-> Beschreibt eure Position

- Arbeiten gerade in einer Senke
	- Stark überwuchert; viel Gestrüpp in der Mitte
	- Dornenbüsche und seltsam blassgrüner Farn

> [!recite|] Beschreibung
>  Die Axtschläge der Holzfäller hallen durch den Wald. Es ist ein Routinejob. [[Torgar Feralis]], der Vorarbeiter, wischt sich den Schweiß von der Stirn.
> "Guter Fortschritt heute," brummt er. "Aber bleibt wachsam. Die Wölfe sind in letzter Zeit aggressiver. Und man erzählt sich, dass tiefer im Wald, Richtung der alten Ruinen, Dinge umgehen, die man besser nicht stört."
- Auf einmal fliegt in einiger Entfernung ein aufgeschreckter Schwarm Vögel auf.

**Für den DM:** 
* Torgar will **nicht** nachsehen ("Wir werden fürs Holzhacken bezahlt, nicht für Heldenmut!"), aber die SCs müssen sicherstellen, dass keine Gefahr droht.
![[Reiter-Patrouille]]
![[Goldene Garbe]]
![[Schlammschlacht]]
![[Schwarze Garde]]

![[Toter Hirsch]]
![[Nebel-Bote]]
Keine unbenutzten Begegnungen für "Alle Orte" gefunden.



---

### 2. Der Kampf
**Gegner (CR 3 Encounter):**
* **1x Eulenbär (Owlbear)**
    * *Verhalten:* Ein ganz normaler, massiver Eulenbär. Er ist wütend und panisch wegen des Einschlags direkt neben seiner Höhle. Er greift alles an, was sich bewegt.
* **2x Wölfe** (Optional, wenn die Gruppe gut drauf ist)
    * Sie nutzen das Chaos, um von der Flanke anzugreifen, fliehen aber, wenn der Eulenbär fällt.

**Taktik:**
* Der Eulenbär nutzt seine Multiattacke. Er ist *nicht* mutiert, aber extrem gefährlich durch reine Muskelkraft.
* Das Gelände ist schwieriges Terrain (Unterholz, umgestürzte Bäume).

---

## 3. Der Fund: Der Krater
Nach dem Kampf finden die Spieler die Ursache des Lärms auf einer kleinen Lichtung oder direkt neben der Eulenbär-Höhle.

**Der Ort:**
* Ein frischer, rauchender **Einschlagkrater**, etwa zwei Meter breit.
* Die Erde ist aufgewühlt, das Gras am Rand ist versengt (aber nicht mutiert).

**Das Objekt:**
* In der Mitte der Grube liegt der **[[Schattensplitter]]**.
* Er sieht aus wie ein Stück Obsidian, durchzogen von pulsierenden violetten Adern.
* Er summt leise. Es ist eine unnatürliche, magische Resonanz.

**Checks:**
* *Arcana (DC 13):* Das Objekt strahlt starke Magie aus. Es erinnert entfernt an die Geschichten über die fliegenden Städte, wirkt aber... instabil.
* *Perception (DC 12):* Wer genau hinsieht, merkt, dass die Schatten um den Splitter herum "falsch" fallen. Sie sind dunkler und länger, als die Sonne es zulassen sollte.

---

## 4. Entscheidung in Ashabenford
Die Holzfäller packen sofort zusammen ("Das bringt Unglück!"). Zurück in [[Wissen - Ashabenford]] haben die SCs nun dieses Objekt.

Da es keine offensichtliche Mutation am Splitter gibt, ist er vor allem eins: **Geheimnisvoll und potenziell wertvoll.**

Die drei Hooks bleiben bestehen, aber der Kontext ändert sich:
1.  **Zhentarim (Kaelen):** Will das "Meteoriten-Metall" kaufen. Er hält es für eine seltene Ressource.
2.  **Drow (Vgetarnt):** Spürt die magische Signatur des Splitters (Drow-Magie/Schattenfell) und will ihn "sicherstellen".
3.  **Rat (Andra):** Ist besorgt, weil kurz nach dem Einschlag Berichte über Unruhe im Wald eintreffen. Sie bittet die Helden, den Splitter sicher zu verwahren, bis man weiß, was es ist.

---